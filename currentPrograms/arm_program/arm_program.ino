...
const int NUM_SERVOS = 4; //How many servos are on the actual robot.

int PIN[NUM_SERVOS], INITANGLE[NUM_SERVOS], ANA[NUM_SERVOS], DIG[3], ARM[3][2][2], WRIST[2], CLAW[2], baseIndex, clawIndex, onOff, offset[3];
Servo myservo[NUM_SERVOS];

const int NUM_BASE_POSITIONS = 5; //Number of pre-set base positions
const int BASE[NUM_BASE_POSITIONS] = {0, 45, 90, 135, 180};
...
void setup()   {
	...
	//Now we attach all the servos to the respective pins
	//So that we can get information from the right pins
...
void basePos()
{
	...
	Serial.print("Base: ");
	Serial.print(value);
	if (value < LOW_JOYSTICK_VALUE)
	{
		baseIndex++;
		if (baseIndex > 4)
			baseIndex = 4;
	}
	else if (value > HIGH_JOYSTICK_VALUE)
	{
		baseIndex--;
		if (baseIndex < 0)
			baseIndex = 0;
...
void horzPos()
{
	...
	if (value > HIGH_JOYSTICK_VALUE)
	{
		if (WRIST[0] == 0)
			WRIST[0] = 1;
	}
	else if (value < LOW_JOYSTICK_VALUE)
	{
		if (WRIST[0] == 1)
			WRIST[0] = 0;
	}
